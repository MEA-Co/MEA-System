...
const ADMIN_VIEW_COOKIE = 'mea-admin-view';

// Presentation only: authorization always uses the real profile role.
export async function getViewRole(actualRole: MemberRole): Promise<MemberRole> {
  if (actualRole !== 'admin') return actualRole;
  const value = (await cookies()).get(ADMIN_VIEW_COOKIE)?.value;
...
export async function setAdminView(role: string): Promise<{ error?: string }> {
  ...
  if (!access.user || !access.isOnboarded || access.role !== 'admin') {
    return { error: '관리자만 역할 화면을 전환할 수 있어요.' };
  }
  if (!MEMBER_ROLES.some((candidate) => candidate === role)) {
    return { error: '선택한 역할을 확인해 주세요.' };
  }
  (await cookies()).set(ADMIN_VIEW_COOKIE, role, {
    httpOnly: true,
    sameSite: 'lax',
