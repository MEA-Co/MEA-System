...
import { isConsultingReportRequest } from '@/features/consulting/report';
import { createConsultingReportDocument } from '@/features/consulting/report/server';
...
export async function POST(request: Request) {
  ...
  if (!isConsultingReportRequest(body)) {
    return NextResponse.json(
  ...
  try {
    const pdfBuffer = await renderToBuffer(
      createConsultingReportDocument(body.document),
    );
