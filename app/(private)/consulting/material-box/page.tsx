...
export default async function MaterialBoxConsultingPage() {
  const { role: actualRole } = await requireUserAccess({
  ...
  return (
    <main className="min-h-svh bg-muted/30">
      ...
      <header className="border-b bg-background">
        <div className="mx-auto flex min-h-16 max-w-7xl flex-wrap items-center justify-between gap-4 px-4 py-3 md:px-6 lg:px-8">
      ...
      <div className="mx-auto max-w-7xl space-y-4 px-4 py-5 md:px-6 md:py-7 lg:px-8">
        <MaterialBoxConsulting
          role={role}
          persistCompletion={actualRole === 'student'}
          debug={role === 'admin'}
          reviewEnabled={
            role === 'admin' ||
            role === 'consultant' ||
