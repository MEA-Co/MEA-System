...
} from '@/app/(private)/consulting/demo/_lib/types';
...
function getSubmittedValue(memory: ConsultingMemory<MergeSortContext>) {
...
function getValidationResult(
  memory: ConsultingMemory<MergeSortContext>,
  nodeId: string,
) {
...
function getLastError(memory: ConsultingMemory<MergeSortContext>) {
  return memory.lastToolError?.message ?? '작업을 처리하지 못했습니다.';
}

export const mergeSortPlan = defineConsultingPlan<
  MergeSortContext,
  MergeSortTools
>({
