-- Existing scale questions keep the five-point defaults; other question kinds ignore this setting.
create function private.scale_config_valid(p_config jsonb, p_ready boolean default false)
returns boolean language plpgsql immutable set search_path='' as $$
declare max_score integer;
begin
  if jsonb_typeof(p_config) is distinct from 'object'
    or jsonb_typeof(p_config->'max') is distinct from 'number'
    or (p_config->>'max') !~ '^[2-9]$'
    or jsonb_typeof(p_config->'low') is distinct from 'string'
    or jsonb_typeof(p_config->'middle') is distinct from 'string'
    or jsonb_typeof(p_config->'high') is distinct from 'string'
    or jsonb_typeof(p_config->'allowText') is distinct from 'boolean'
    or length(p_config->>'low') > 500
    or length(p_config->>'middle') > 500
    or length(p_config->>'high') > 500 then return false;
  end if;
  max_score := (p_config->>'max')::integer;
  return not p_ready or (
    btrim(p_config->>'low') <> ''
    and btrim(p_config->>'high') <> ''
    and (max_score % 2 = 0 or btrim(p_config->>'middle') <> '')
  );
end $$;
revoke all on function private.scale_config_valid(jsonb,boolean) from public,anon,authenticated;

alter table public.questionnaire_questions
  add column scale_config jsonb not null default '{"max":5,"low":"전혀 그렇지 않다","middle":"보통이다","high":"매우 그렇다","allowText":false}'::jsonb,
  add constraint questionnaire_scale_config_check
    check (kind <> 'scale' or private.scale_config_valid(scale_config));
...
create function private.scale_answer_valid(p_config jsonb,p_value text,p_complete boolean)
returns boolean language plpgsql immutable set search_path='' as $$
declare answer jsonb; score integer;
begin
  if p_value is null then return false; end if;
  if btrim(p_value) = '' then return not p_complete; end if;
  if p_value ~ '^[1-9]$' then
    return p_value::integer <= (p_config->>'max')::integer;
  end if;
  if p_config->'allowText' <> 'true'::jsonb then return false; end if;
  ...
  if jsonb_typeof(answer) is distinct from 'object'
    ...
    or length(answer->>'text') > 5000 then return false;
  end if;
  score := (answer->>'score')::integer;
  return score <= (p_config->>'max')::integer;
end $$;
revoke all on function private.scale_answer_valid(jsonb,text,boolean) from public,anon,authenticated;

create function private.scale_answer_text(p_config jsonb,p_value text)
returns text language plpgsql immutable set search_path='' as $$
declare answer jsonb; score integer; label_text text; extra text := '';
begin
  if p_value = '' then return ''; end if;
  if p_value ~ '^[1-9]$' then score := p_value::integer;
  else
    answer := p_value::jsonb;
    score := (answer->>'score')::integer;
    extra := answer->>'text';
  end if;
  label_text := case
    when score = 1 then p_config->>'low'
    when score = (p_config->>'max')::integer then p_config->>'high'
    when (p_config->>'max')::integer % 2 = 1 and score = ((p_config->>'max')::integer + 1) / 2 then p_config->>'middle'
    else '' end;
  return score::text || '점'
    || case when label_text <> '' then ' · ' || label_text else '' end
    || case when btrim(extra) <> '' then E'\n추가 답변: ' || extra else '' end;
end $$;
revoke all on function private.scale_answer_text(jsonb,text) from public,anon,authenticated;
...
AS $function$
  select jsonb_build_object('questionnaireId',v.questionnaire_id,'versionId',v.id,'title',v.title,'revision',v.revision,'savedAt',v.updated_at,
    'sections',coalesce((select jsonb_agg(jsonb_build_object('id',s.id,'title',s.title,'questions',coalesce((
      select jsonb_agg(jsonb_build_object('id',q.id,'logicalKey',q.logical_key,'text',q.body,'kind',q.kind,'options',q.options,'scaleConfig',q.scale_config,'details',coalesce((
...
begin
 ...
 if not (case when question_record.kind='scale' then private.scale_answer_valid(question_record.scale_config,item.value#>>'{}',p_complete) else private.question_answer_valid(question_record.kind,question_record.options,item.value#>>'{}',p_complete) end) then
 ...
 values(rid,p_version_id,item.key::uuid,case when question_record.kind='scale' then private.scale_answer_text(question_record.scale_config,item.value#>>'{}') else private.question_answer_text(question_record.kind,question_record.options,item.value#>>'{}') end,selected)
