-- Question blocks are a standalone authoring library, not questionnaire content.
create table public.question_blocks (
  ...
  row_mode text not null check (row_mode in ('single', 'repeatable', 'reference')),
  max_rows integer check (max_rows between 1 and 20),
  source_block_id uuid references public.question_blocks(id) on delete restrict,
  source_field_id uuid,
  after_block_id uuid references public.question_blocks(id) on delete restrict,
  condition jsonb,
  revision integer not null check (revision > 0),
  last_save_id uuid not null,
  last_save_hash text not null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  archived_at timestamptz,
  check (
    (row_mode = 'repeatable' and max_rows is not null and source_block_id is null and source_field_id is null)
    or (row_mode = 'reference' and max_rows is null and source_block_id is not null and source_field_id is not null)
    or (row_mode = 'single' and max_rows is null and source_block_id is null and source_field_id is null)
  )
);
create index question_blocks_creator_idx on public.question_blocks(created_by);
create index question_blocks_source_idx on public.question_blocks(source_block_id) where source_block_id is not null;
create index question_blocks_after_idx on public.question_blocks(after_block_id) where after_block_id is not null;
create index question_blocks_active_updated_idx on public.question_blocks(updated_at desc) where archived_at is null;
...
create policy "Staff read question blocks" on public.question_blocks
for select to authenticated
using ((select private.is_admin()) or (select private.is_consultant_lead()));

create function private.validate_question_block() returns trigger
    ...
    if v_field->>'kind' in ('single', 'multiple') then
      ...
      for v_option in select value from pg_catalog.jsonb_array_elements(v_field->'options') loop
        if pg_catalog.jsonb_typeof(v_option) is distinct from 'object'
          ...
          or char_length(pg_catalog.btrim(coalesce(v_option->>'label', ''))) not between 1 and 100 then
...
revoke all on function private.validate_question_block() from public, anon, authenticated;
create trigger validate_question_block before insert or update on public.question_blocks
for each row execute function private.validate_question_block();

create function private.save_question_block(p_document jsonb, p_expected_revision integer, p_save_id uuid)
...
begin
  ...
  if p_document is null or pg_catalog.jsonb_typeof(p_document) <> 'object'
    ...
    or char_length(p_document::text) > 60000 then
...
revoke all on function private.save_question_block(jsonb, integer, uuid) from public, anon;
grant execute on function private.save_question_block(jsonb, integer, uuid) to authenticated;

create function public.save_question_block(p_document jsonb, p_expected_revision integer, p_save_id uuid)
returns jsonb language sql security invoker set search_path = '' as $$
  select private.save_question_block(p_document, p_expected_revision, p_save_id);
$$;
revoke all on function public.save_question_block(jsonb, integer, uuid) from public, anon;
grant execute on function public.save_question_block(jsonb, integer, uuid) to authenticated;

create function private.archive_question_block(p_id uuid, p_expected_revision integer)
returns jsonb language plpgsql security definer set search_path = '' as $$
declare v_block public.question_blocks%rowtype;
begin
  if auth.uid() is null or not (private.is_admin() or private.is_consultant_lead()) then
    raise exception 'Question block author permission required' using errcode = '42501';
  end if;
  ...
  select * into v_block from public.question_blocks where id = p_id for update;
  if not found then raise exception 'Question block not found' using errcode = 'P0002'; end if;
  if v_block.created_by <> auth.uid() and not private.is_admin() then
    raise exception 'Only author or admin can archive' using errcode = '42501';
  end if;
  if v_block.archived_at is not null and v_block.revision = p_expected_revision + 1 then
    return pg_catalog.to_jsonb(v_block);
  end if;
  if v_block.archived_at is not null or v_block.revision <> p_expected_revision then
    raise exception 'Question block revision conflict' using errcode = '40001';
  end if;
  update public.question_blocks set archived_at = now(), revision = revision + 1
  where id = p_id returning * into v_block;
  return pg_catalog.to_jsonb(v_block);
end $$;
revoke all on function private.archive_question_block(uuid, integer) from public, anon;
grant execute on function private.archive_question_block(uuid, integer) to authenticated;

create function public.archive_question_block(p_id uuid, p_expected_revision integer)
returns jsonb language sql security invoker set search_path = '' as $$
  select private.archive_question_block(p_id, p_expected_revision);
$$;
revoke all on function public.archive_question_block(uuid, integer) from public, anon;
grant execute on function public.archive_question_block(uuid, integer) to authenticated;
