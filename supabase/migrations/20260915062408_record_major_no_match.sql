-- Explicit student no-match feedback, saved atomically with the shown search.
begin;
create function public.record_major_no_match(
...
begin
  if p_user_id is null or p_candidates is null
    or jsonb_typeof(p_candidates) <> 'array' then
    raise exception 'Invalid no_match';
  end if;
  insert into public.major_search_requests(id,session_id,user_id,input_text,normalized_input,
    status,result_source,candidates,catalog_version,completed_at)
  values(p_request_id,p_session_id,p_user_id,p_input_text,p_normalized_input,
    'completed','db',p_candidates,p_catalog_version,now())
  ...
  if r.user_id is distinct from p_user_id or r.session_id is distinct from p_session_id
    ...
    or r.candidates is distinct from p_candidates or r.catalog_version is distinct from p_catalog_version
    or r.status <> 'completed' or r.result_source is distinct from 'db'
    or r.model_name is not null then raise exception 'No-match conflict'; end if;
  if exists(select 1 from public.major_search_feedback where request_id=p_request_id and action <> 'no_match')
    then raise exception 'Request already used'; end if;
  insert into public.major_search_feedback(request_id,action,major_id)
    values(p_request_id,'no_match',null) on conflict do nothing;
  return jsonb_build_object('requestId',p_request_id);
end;
$$;
revoke all on function public.record_major_no_match(uuid,uuid,uuid,text,text,jsonb,text) from public,anon,authenticated;
grant execute on function public.record_major_no_match(uuid,uuid,uuid,text,text,jsonb,text) to service_role;
create or replace function public.confirm_major_search(
  p_request_id uuid, p_user_id uuid, p_session_id uuid,
  p_input_text text, p_normalized_input text, p_candidates jsonb,
