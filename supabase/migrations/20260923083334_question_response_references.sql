-- Question-wide answered conditions and response-row references.
-- Keep legacy field-specific references readable; no existing data is rewritten.
alter table public.questions drop constraint questions_check;
alter table public.questions add constraint questions_check check (
 ...
 or (row_mode = 'reference' and max_rows is null and source_block_id is not null)
...
begin
  ...
  if new.source_block_id is not null then
    ...
    if new.source_field_id is null then
      if not exists (select 1 from jsonb_array_elements(coalesce(new.condition->'clauses', '[]'::jsonb)) clause
        where clause->>'blockId' = new.source_block_id::text and clause->>'op' = 'answered'
          and clause->>'fieldId' is null) then
        raise exception 'Referenced question needs an answered condition' using errcode = '22023';
      end if;
    else
