...
begin
 ...
 choices:=jsonb_build_array(jsonb_build_object('id',one,'label','팀 프로젝트'),jsonb_build_object('id',two,'label','발표'));
 ...
 answers:=jsonb_build_object(qtext::text,'자유로운 서술 답변',qscale::text,'3',qsingle::text,one::text,qmulti::text,jsonb_build_array(two,one)::text);
 ...
 if not exists(select 1 from public.questionnaire_answers where question_id=qsingle and body='팀 프로젝트' and selection=to_jsonb(one::text)) then raise exception 'Single not readable';end if;
 if not exists(select 1 from public.questionnaire_answers where question_id=qmulti and body='팀 프로젝트, 발표' and selection=jsonb_build_array(two,one)) then raise exception 'Multiple not readable';end if;
...
reset role;
select 'Question types, readable answers, stable IDs and validation passed' as result;
rollback;
