...
- 관리자와 컨설턴트 리드의 운영 관리에는 질문지 관리(`?view=questionnaire`)가 표시된다. `QuestionnaireView`의 기본 화면은 질문지 목록과 새 질문지 제작 버튼이며, `draft=new`는 새 편집기, `draft=<버전 UUID>`는 저장된 초안을 연다. 편집 화면에서 목록으로 돌아갈 수 있고 `QuestionnaireEditor`에서 제목·섹션·질문을 편집한다. `QuestionDetailsEditor`의 설명 항목은 제목·본문·컨설턴트 공개 여부를 가지며 기본 비공개다. 편집/컨설턴트 미리보기 탭은 중립색이며 공개 스위치는 파란색과 공개/비공개 문구를 사용한다. 컨설턴트 미리보기에는 공개 항목만 표시한다. 관리자는 관리자 화면에서도 질문지 관리에 직접 접근할 수 있다.
