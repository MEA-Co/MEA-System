...
- 질문 카드의 순서는 유형 선택·삭제 버튼 → 질문 입력 → 답변 UI/선택지 설정이다. 척도는 가로선 위의 점을 선택하며 편집 중에는 비활성 미리보기를 표시한다. 선택형 options의 `isOther`는 질문당 하나만 허용하고, 해당 답변은 `{id,text}`(다수 선택은 일반 ID와 객체의 배열)로 전달한다. 기타 내용은 최대 5,000자이며 임시 저장은 공백을 허용하지만 완료 시 필수다. 기존 `selection`과 사람이 읽을 수 있는 `body`를 유지하며 body에는 `기타: 내용`으로 저장한다. 검증: `supabase/tests/questionnaire_other_choices.sql`, `scripts/verify-questionnaire-question-types.mjs`.
