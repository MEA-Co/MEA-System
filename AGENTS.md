...
- 컨설턴트 리드의 운영 관리에는 질문지 제작(`?view=questionnaire`)이 표시된다. `QuestionnaireWorkspace`가 저장된 초안을 불러오고 `QuestionnaireEditor`에서 제목·섹션·질문을 편집한다. `QuestionDetailsEditor`의 설명 항목은 제목·본문·컨설턴트 공개 여부를 가지며 기본 비공개다. 편집/컨설턴트 미리보기 탭은 중립색이며 공개 스위치는 파란색과 공개/비공개 문구를 사용한다. 컨설턴트 미리보기에는 공개 항목만 표시한다. 관리자도 리드 표시 역할로 전환하여 이용한다.
- 질문지 저장은 `features/questionnaires/{types,schema,server,save-session}.ts`, 대시보드 `_actions/save-questionnaire.ts`, `_hooks/useQuestionnaireSave.ts`에 둔다. 변경 시 10초 간격 자동 저장과 수동 저장, 저장한 질문지 열기/새 질문지를 제공한다. UUID 유지·revision 충돌 검사·동일 요청 재시도로 덮어쓰기와 중복 생성을 방지한다. DB는 질문지/버전/섹션/질문/설명/응답/답변을 분리하며 아직 배포·배정·답변 쓰기 UI/RPC는 제공하지 않는다. 실제 관리자/리드만 초안을 저장하며 표시 역할 쿠키는 권한 근거가 아니다. 자세한 구조와 검증 범위는 `docs/questionnaire-storage-design.md`, 테스트는 `scripts/verify-questionnaire-storage.mjs`와 `supabase/tests/questionnaire_storage.sql`을 참고한다.
