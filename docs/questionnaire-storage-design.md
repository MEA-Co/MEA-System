...
`questionnaire_questions.kind`는 `text`(기본), `scale`, `single`, `multiple`이다. 선택지는 같은 행의 `options` JSONB 배열 `{id,label}`로 저장한다. 유형이 없던 기존 문서는 서술형으로 읽힌다. 초안 저장은 빈 선택지를 허용하지만 게시/배포 시 2~20개의 이름이 서로 다른 선택지를 검증한다. 5점 척도는 고정된 1~5점과 문구를 사용한다. 배포 후 기존 콘텐츠 잠금이 유형·선택지에도 적용된다.
...
선택형 options에는 `isOther?: boolean`을 저장하며 true는 질문당 하나만 허용한다. 일반 선택은 기존 UUID 문자열, 기타 선택은 `{id,text}`를 JSON 문자열로 전달하고 다수 선택은 두 형태를 섞은 배열로 전달한다. 기타 내용은 5,000자 이하이며 작성 중에는 빈 내용을 저장할 수 있지만 완료 시에는 입력해야 한다. 기존 answers.selection 저장 형식(단일은 문자열, 다수는 JSON 배열)을 유지한다. body에는 `기타: 입력 내용`을 포함하므로 AI 전달 시 별도 선택지 조회 없이 사용 가능하다. 질문별 답변 ID, revision, 자동 저장·완료 잠금은 유지한다.
