...
- `QuestionnaireWorkspace`에서 저장한 초안을 선택하거나 새 질문지를 시작한다. `draft` URL 매개변수는 버전 ID다. 기본 진입은 가장 최근 저장한 초안을 열며, `draft=new`는 DB 쓰기 없이 빈 문서를 준비한다. 초안은 관리자/리드가 공유한다.
- 수정 사항이 있을 때 10초 간격으로 자동 저장하고 수동 저장도 제공한다. 저장 중 추가 입력은 다음 저장 대상으로 남는다. 중복 저장을 막고 응답을 받지 못한 요청은 동일 saveId·revision·문서로 재시도한다. 새 문서가 처음 저장되면 URL과 목록을 갱신한다.
...
- 배포 버전과 그 하위 행의 변경/삭제는 DB에서도 차단한다. 초안만 편집·하위 항목 삭제 가능하다. 배포 버전과 응답의 연결은 삭제 제한 FK로 보존하고 질문지는 archive 처리한다.
