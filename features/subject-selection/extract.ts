...
import { convertHwpToPdf, parseDocument } from './document';
import {
  extractionSchema,
  type ImportContext,
  type ImportResult,
...
track이 과학중점 과정이면 해당 표만 추출한다. 이 경우 track_matched는 요청 과정 표를 실제로 선택했을 때만 true이고, track_evidence에는 같은 표 안의 과정 표지 또는 그 과정을 식별하는 선택·이수 조건을 남긴다. 과학중점 과정은 과학 교과 선택 및 수학·과학·정보 이수 조건이 같은 표에 있는지를 확인한다. 과정 근거를 확인하지 못하면 track_matched=false, track_evidence=null, table_found=false, curriculum=[]로 반환한다. 일반 과정 또는 track이 비어 있으면 과정 표지를 별도로 확인하지 않아도 된다. 대상 입학년도와 학년·학기 열이 맞는 편제표를 우선 추출하고 track_matched와 track_evidence는 null로 반환한다.
...
HWP 원문 데이터에 tables가 있으면 cells의 row, col, rowSpan, colSpan으로 표를 복원한다. cells에 없는 좌표는 빈 셀이다. sections의 텍스트 레코드보다 tables의 셀 좌표와 텍스트를 우선한다.
...
export async function extractCurriculum(
  ...
  const conversion = await convertHwpToPdf(bytes);
  const analysisBytes = conversion?.pdf ?? bytes;
  const parsed = parseDocument(analysisBytes);
  ...
  if (parsed.text === null)
    content.push({
      ...
      file_data: `data:application/pdf;base64,${analysisBytes.toString('base64')}`,
    });
  else
    content.push({ type: 'input_text', text: `원문 데이터:\n${parsed.text}` });
  const response = await client.responses.parse({
    model: process.env.SUBJECT_EXTRACTION_MODEL || 'gpt-5.6-sol',
